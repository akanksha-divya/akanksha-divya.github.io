...
/*setInterval(function randomFunc() {
  ...
  if(random>=1 &&random<=3){
    ...
    //
    //coins.style.animation="coinani 5.5s infinite";
    coins.style.visibility="visible";
} 
else if(random>=4 &&random<=6){
    mask.style.visibility="visible";
    oxygen.style.visibility="hidden";
    //mask.style.animation="maskani 5.5s infinite";
    coins.style.visibility="hidden";
}
else{
    mask.style.visibility="visible";
    //oxygen.style.animation="virus 5.5s infinite";
    coins.style.visibility="hidden";
...
/*setInterval(function randomVirus() {
    if(val%3==0){
        virus.style.visibility="visible";
        virus.style.animationDuration="newdur"
        virus2.style.visibility="hidden";
    }
    else{
        virus2.style.visibility="visible";
        virus.style.visibility="hidden";
        virus2.style.animationDuration="newdur";
...
function interval() {
    ...
    if ((virusLeft > 45 && virusLeft < 140 && heroTop > 220 && barvalue != 0) || (virus2Left > 40 && virus2Left < 140 && heroTop > 220 && barvalue > 0)) {
        counter++;
...
function drawhealthbar(canvas, x, y, width, height, health, max_health) {
    ...
    if (health <= 0) {
        ...
        audiogo.play();
        audioJump.pause();
        document.getElementById("score").innerHTML = "Your Score :" + (Math.floor(counter / 100) + (count * 2));
